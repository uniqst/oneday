...
function siteoptions_add_admin() {
    ...
    $tt_page = add_menu_page('Site Options', 'Настройка шаблона', 'edit_theme_options', 'siteoptions','siteoptions_options_page');
...
<div class="wrap" id="truethemes_container">
  <div id="of-popup-save" class="of-save-popup">
    <div class="of-save-save">Настройки сохранены</div>
  </div>
  <div id="of-popup-reset" class="of-save-popup">
    <div class="of-save-reset">Настройки обновлены</div>
  </div>
  <form action="" enctype="multipart/form-data" id="ofform">
    <div id="header">
      <div class="logo">
        <h2><?php $themename = get_theme_data(get_stylesheet_directory() . '/style.css');

$themename = $themename['Name'];
 echo "$themename"; ?> </h2><span class="fg" style="float:right; margin:-25px -550px 0 0;">Wp-city.ru (разработка авторских тем для WordPress) </span>
      </div>
    ...
    <input type="submit" value="Сохранить настройки" class="button-primary" />
  </form>
  <form action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" method="post" style="display:inline" id="ofform-reset">
    <span class="submit-footer-reset">
    <input name="reset" type="submit" value="Скинуть настройки" class="button submit-button reset-button" onclick="return confirm('Вы уверены что хотите скинуть настройки!');" />
...
<script type="text/javascript">
			...
			new AjaxUpload(clickedID, {
				  ...
				  onSubmit: function(file, extension){
						clickedObject.text('Загружаю'); // change button text, when user selects file	
						this.disable(); // If you want to allow uploading only 1 file at time, you can disable upload button
						interval = window.setInterval(function(){
							var text = clickedObject.text();
							if (text.length < 13){	clickedObject.text(text + '.'); }
							else { clickedObject.text('Загружаю'); } 
						}, 200);
				  },
